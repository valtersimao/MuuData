...
public class VacinaDAO implements DAO {
        ...
        try (PreparedStatement trans = c.prepareStatement(sql)) {
            ...
            while (result.next()) {
                Vacina vacina = new Vacina(result.getInt("id_vacina"), result.getString("nome"),
                        ...
                        result.getString("descricao"), (int) id,
    ...
    public boolean existsInHistorico(Vacina vacina) {
        String sql = "SELECT COUNT(*) FROM muudata.historico_vacina WHERE id_historico = ? AND id_vacina = ?";
